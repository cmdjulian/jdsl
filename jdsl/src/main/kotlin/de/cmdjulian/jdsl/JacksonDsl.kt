...
@JsonMarker
sealed class JacksonNodeBuilder<T : JsonNode>(
    internal val node: T,
) {
...
internal class GeneralJacksonNodeBuilder(
    node: JsonNode,
) : JacksonNodeBuilder<JsonNode>(node)

@Suppress("FunctionName", "kotlin:S100", "MemberVisibilityCanBePrivate", "TooManyFunctions")
class JacksonObjectNodeBuilder private constructor(
    node: JacksonObjectNode = JsonNodeFactory.objectNode(),
    ...
    companion object {
        ...
        fun <T> obj(
            transformer: Transformer<T, in ObjectNode>,
            init: JacksonObjectNodeBuilder.() -> Unit,
        ): T = transformer.process(JacksonObjectNodeBuilder().apply(init))
    }
    ...
    fun put(
        key: String,
        value: arr?,
    ) = put(key, value?.invoke())

    fun put(
        key: String,
        value: Boolean?,
    ) = if (value != null) put(key, json(value)) else put(key, `null`)
...
class JacksonArrayNodeBuilder private constructor(
    array: ArrayNode = JsonNodeFactory.arrayNode(),
) : JacksonNodeBuilder<ArrayNode>(array) {
    companion object {
        fun arr(init: JacksonArrayNodeBuilder.() -> Unit): ArrayNode = arr(Transformers.ArrayNode, init)

        fun <T> arr(
            transformer: Transformer<T, in ArrayNode>,
            init: JacksonArrayNodeBuilder.() -> Unit,
        ): T = transformer.process(JacksonArrayNodeBuilder().apply(init))
    }
...
fun <T, U> json(
    transformer: Transformer<T, in JsonNode>,
    generator: () -> U,
): T {
    val node =
        when (val value = generator()) {
            `null` -> `null`
            is JsonNode -> value
...
fun json(value: Number): ValueNode =
    when (value) {
